...
BOOST_AUTO_TEST_CASE(testAreaAfterScaling)
{
  ...
  const double factor = 4.0;
  compSh.scale(factor);
  BOOST_CHECK_CLOSE(areaBefore*factor*factor, compSh.getArea(), epsilon);
}
...
BOOST_AUTO_TEST_CASE(testScaleParametr)
{
  ...
  compSh.add(std::make_shared<Circle>(Circle{{2.4,3.6},4.0}));
  compSh.add(std::make_shared<Triangle>(Triangle{{2.4,3.6},{1.5,5.4},{3.3,5.4}}));
  BOOST_CHECK_THROW(compSh.scale(-1.0),std::invalid_argument);
}

BOOST_AUTO_TEST_CASE(testAddParametr)
{
  CompositeShape compSh;
  BOOST_CHECK_THROW(compSh.add(nullptr),std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(testRemoveParametr)
{
  ...
  compSh.add(std::make_shared<Circle>(Circle{{2.4,3.6},4.0}));
  compSh.add(std::make_shared<Triangle>(Triangle{{2.4,3.6},{1.5,5.4},{3.3,5.4}}));
  BOOST_CHECK_THROW(compSh.remove(20),std::out_of_range);
}
...
BOOST_AUTO_TEST_CASE(testSquareBraketsParametr)
{
  ...
  compSh.add(std::make_shared<Circle>(Circle{{2.4,3.6},4.0}));
  compSh.add(std::make_shared<Triangle>(Triangle{{2.4,3.6},{1.5,5.4},{3.3,5.4}}));
  BOOST_CHECK_THROW(compSh[20],std::out_of_range);
}
...
BOOST_AUTO_TEST_CASE(TestAddShape)
{
  ...
  compSh.add(std::make_shared<Circle>(Circle{{2.4,3.6},4.0}));
  compSh.add(std::make_shared<Triangle>(Triangle{{2.4,3.6},{1.5,5.4},{3.3,5.4}}));
  BOOST_CHECK_EQUAL(compSh.getSize(), 3);
}
...
BOOST_AUTO_TEST_CASE(TestRemoveShape)
{
  ...
  compSh.add(std::make_shared<Triangle>(Triangle{{2.4,3.6},{1.5,5.4},{3.3,5.4}}));
  compSh.remove(1);
  BOOST_CHECK_EQUAL(compSh.getSize(), 2);
}
